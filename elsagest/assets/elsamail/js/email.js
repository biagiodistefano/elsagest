...
const selectConsigli = $('#consigli-destinatari');
const selectConsiglieri = $('#consiglieri-destinatari');
const checkDirectors = $('#check-directors');

$('.cons-destinatari').on('change', evt => {
  const $this = $(evt.currentTarget);
  if (selectConsigli.find('option:selected').val() !== '0' || selectConsiglieri.find('option:selected').val() !== '0') {
    checkDirectors.attr('checked', true);
    checkDirectors.attr('disabled', false);
    checkDirectors.parent().removeClass('text-muted');
    if ($this.find('option:selected').val() !== '0') {
      $($this.attr('data-opposite')).val('0');
      $('#soci-destinatari').val('0');
    }
  } else {
    checkDirectors.attr('checked', false);
    checkDirectors.attr('disabled', true);
    checkDirectors.parent().addClass('text-muted');
  }
});
